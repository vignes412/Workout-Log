import React, { memo, ChangeEvent, MouseEvent, useMemo } from "react";
import {
  Box,
  TextField,
  Button,
...
import { fieldGroups, MeasurementGroup, MeasurementKey } from "../../constants/measurementConstants";
...
interface MeasurementFormProps {
  ...
  handleUnitChange: (event: MouseEvent<HTMLElement>, newUnit: "metric" | "imperial" | null) => void;
  toggleQuickEntry: (e: ChangeEvent<HTMLInputElement>) => void;
  handleDateChange: (newValue: Dayjs | null, context?: any) => void;
...
const MeasurementForm: React.FC<MeasurementFormProps> = ({
  ...
  // Convert Date object to Dayjs object if needed
  const dateValue = useMemo(() => {
    if (!measurements.date) return null;
    // Check if it's already a Dayjs object
    if ((measurements.date as any).format) return measurements.date as Dayjs;
    // If it's a Date object, convert it to Dayjs
    return dayjs(measurements.date);
  }, [measurements.date]);
            ...
            >
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  >
                    {(group as string).replace(/([A-Z])/g, " $1")}
                  </Typography>
                </Box>
              </AccordionSummary>
              <AccordionDetails>
                <Grid container spacing={2}>
                  {fields.map((field) => (
                    <Grid item xs={12} sm={6} md={4} key={field}>
                      <MeasurementField 
                        field={field} 
                        value={measurements[field] as string} 
                        onChange={handleInputChange}
                        unit={unit}
                      />
                    </Grid>
...
export default memo(MeasurementForm);