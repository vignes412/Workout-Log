...
import { MeasurementKey, MeasurementGroup } from "../../constants/measurementConstants";
import { ChartData, ChartOptions } from 'chart.js';

interface MeasurementChartProps {
  logs: [string, ...any[]][];
  selectedMeasurements: Record<MeasurementGroup, MeasurementKey[]>;
  handleSelectionChange: (group: MeasurementGroup, fields: MeasurementKey[]) => void;
  themeMode: string;
...
const MeasurementChart: React.FC<MeasurementChartProps> = ({
  ...
  const dynamicChartData = useMemo((): ChartData<"bar"> => {
    const chartData = {
      ...
      datasets: Object.entries(selectedMeasurements).flatMap(([group, fields], groupIndex) =>
        fields.map((field, fieldIndex) => {
          ...
          return {
            label: String(measurementLabels[field] || field),
            data: logs.map(
              (log) => parseFloat(log[Object.keys(initialMeasurements).indexOf(String(field))]) || 0
            ),
  ...
  const dynamicChartOptions = useMemo((): ChartOptions<"bar"> => ({
    responsive: true,
    ...
    scales: {
      ...
      y: {
        ...
        title: {
          ...
          font: {
            size: isMobile ? 10 : 12,
            weight: 500, // Changed from 'medium' to numeric value 500
          },
  ...
  const handleSelectionChangeTyped = (group: MeasurementGroup, value: unknown) => {
    handleSelectionChange(group, value as MeasurementKey[]);
  };
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={2}>
        {(Object.keys(fieldGroups) as MeasurementGroup[]).map((group) => (
          <Grid item xs={12} sm={6} key={group}>
            ...
            >
              ...
              <Select
                ...
                onChange={(e: SelectChangeEvent<unknown>) => handleSelectionChangeTyped(group, e.target.value)}
                label={group.replace(/([A-Z])/g, " $1")}
                renderValue={(selected) => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {(selected as MeasurementKey[]).map((field) => (
                      <Chip
                        key={field}
              ...
              >
                {fieldGroups[group]
                  .filter(field => Object.keys(measurementLabels).includes(String(field)))
                  .map((field) => (
                    <MenuItem 
                      key={field} 
                      value={field}
