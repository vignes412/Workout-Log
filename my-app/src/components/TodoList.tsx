...
const TodoList: React.FC = () => {
  ...
  const fetchAndSyncTodos = useCallback(async (): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await fetchTodos();
      // Sync both completed and not completed todos
      if (response.success && response.data) {
        setTodos(response.data.filter((todo: TodoItem) => !todo.completed) || []);
        setCompletedTodos(response.data.filter((todo: TodoItem) => todo.completed) || []);
      } else {
        setTodos([]);
        setCompletedTodos([]);
      }
  ...
  const addTodo = useCallback(async (): Promise<void> => {
    ...
    const newEntry: TodoItem = { text: newTodo, completed: false };
  ...
  const toggleTodo = useCallback(async (todo: TodoItem, isCompleted: boolean): Promise<void> => {
    const updatedTodo: TodoItem = { ...todo, completed: isCompleted };
