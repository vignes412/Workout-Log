...
export interface EditLog extends Partial<WorkoutLog> {
  originalIndex?: number;
}
...
interface WorkoutLogModalProps {
  ...
  isOffline: boolean;
  editLog?: EditLog;
  onSave?: (row: string[], originalIndex?: number | null, exerciseIndex?: number, setIndex?: number) => Promise<void>;
}
        ...
        try {
          if (editLog.date) {
            if (typeof editLog.date === 'string') {
              try {
                const dateObj = new Date(editLog.date);
            ...
            } else if (editLog.date && typeof editLog.date === 'object' && 'toISOString' in editLog.date) {
              initialDate = (editLog.date as Date).toISOString().split('T')[0];
            }
          }
        } catch (error) {
    ...
    try {
      if (editLog && onSave && editLog.originalIndex !== undefined) {
        try {
          ...
          throw new Error(`Failed to save workout: ${innerError instanceof Error ? innerError.message : 'Unknown error'}`);
        }
      } else {
        try {
          ...
          try {
            ...
            await cacheData("/api/workout", [...(Array.isArray(cachedData) ? cachedData : []), row]);
          } catch (cacheError) {
          ...
          throw new Error(`Failed to log workout: ${dataError instanceof Error ? dataError.message : 'Unknown error'}`);
        }
      }
...
export default React.memo(WorkoutLogModal);