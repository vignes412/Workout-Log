...
const TodaysWorkout = ({ accessToken, onNavigate, toggleTheme, themeMode }) => {
  ...
  // Start rest timer automatically when workout loads
  useEffect(() => {
    if (workout && !restTimerActive && !isFirstSet) {
      // Automatically start rest timer when workout is loaded
      setRestStartTime(Date.now());
      setRestTimerActive(true);
    }
  }, [workout, restTimerActive, isFirstSet]);
