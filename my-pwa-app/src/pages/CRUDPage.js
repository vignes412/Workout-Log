...
const initClient = (callback) => {
  const { API_KEY, CLIENT_ID, DISCOVERY_DOCS, SCOPES } = config.google;
  gapi.load("client", () => {
    gapi.client
      .init({
        apiKey: API_KEY,
...
const CRUDPage = ({ isAuthenticated, setIsAuthenticated }) => {
  ...
  return (
    <div>
      ...
      {data.length > 0 ? (
        ...
        <p>{isOffline ? "Offline - No data cached" : "No data available"}</p>
      )}
    </div>
  );
};
